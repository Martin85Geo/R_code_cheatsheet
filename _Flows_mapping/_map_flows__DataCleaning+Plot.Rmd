...
### revise the center location of Hebei to look nicer ----------------------
...
# net flow ----------------------------------------------------------------
dt01 <- dt1 %>% 
  mutate(id_out = gsub(pattern = ' ', '', paste0(from, '_', to)), ## 31*30=930
...
for (p in prov.list){
  ...
  print(p)
  ### select one province, from which flow out to other provinces
  dt03 <- dt02 %>% 
    dplyr::filter(from == p) %>% 
    dplyr::select(-id_in) # test 'Anhui' ## remove "od_in", keep 'id_out'
  # Rename 
  names(dt03)[names(dt03)=="num"] <- "num_out"
  
  ### select one province, to which flow in from other provinces
  dt04 <- dt02 %>% 
    dplyr::filter(to == p) %>% 
    dplyr::select(num, id_in) 
  # Rename 
...
dt_NetFlow_over0 <- dt_NetFlow %>% 
  filter(num_net_out >= 0) %>% ## only keep net flow > 0
  mutate(mark = 
...
data1 <- data %>% 
  left_join(data, by = c("from" = "to", "to" = "from")) %>%
...
flows <- dt.oi %>%
  # dt_NetFlow_over0 %>%
  ## rescale the width of the flows
  dplyr::mutate(width = scales::rescale(num_net_out, to = c(0, 10))) %>% ## 20
  dplyr::mutate(region = 
  ...
  e_lines(
    ...
    lineStyle = list(
      ...
      color = list(
        ...
        y2 = 1,
        colorStops = list(
          list(offset = 0, color = 'red'),  # color at 0% position
          list(offset = 1, color = 'blue') # color at 100% position
        ),
        global = FALSE
      )
    )
